...
async function parsePRD(options = {}) {
  const prdPath = options.prdPath || (CONFIG?.prdPath) || path.join(process.cwd(), 'scripts/prd.txt');
...
async function updateTasks(options = {}) {
  const tasksPath = options.tasksPath || (CONFIG?.tasksPath);
...
async function generateTaskFiles(options = {}) {
  const tasksPath = options.tasksPath || (CONFIG?.tasksPath);
  const outputDir = options.outputDir || (CONFIG?.taskFilesDir) || path.join(process.cwd(), 'tasks/files');
...
async function analyzeTaskComplexity(options = {}) {
  const tasksPath = options.tasksPath || (CONFIG?.tasksPath);
  const outputPath = options.outputPath || (CONFIG?.complexityReportPath) || path.join(process.cwd(), 'tasks/complexity-report.json');
