...
const STATUS_TRANSITIONS = {
  [TASK_STATUSES.PENDING]: [TASK_STATUSES.IN_PROGRESS, TASK_STATUSES.DEFERRED],
  [TASK_STATUSES.IN_PROGRESS]: [TASK_STATUSES.REVIEW, TASK_STATUSES.PENDING, TASK_STATUSES.DEFERRED],
  [TASK_STATUSES.REVIEW]: [TASK_STATUSES.DONE, TASK_STATUSES.IN_PROGRESS, TASK_STATUSES.PENDING],
  [TASK_STATUSES.DONE]: [TASK_STATUSES.PENDING, TASK_STATUSES.REVIEW],
  [TASK_STATUSES.DEFERRED]: [TASK_STATUSES.PENDING, TASK_STATUSES.IN_PROGRESS]
};
...
class StateStore extends EventEmitter {
  ...
  updateTask(id, updates, options = {}) {
    ...
    if (updates.status && this.state.tasks[taskIndex].status !== updates.status) {
      if (!this.isValidStatusTransition(this.state.tasks[taskIndex].status, updates.status)) {
        log(`Invalid status transition: ${this.state.tasks[taskIndex].status} -> ${updates.status}`, 'error');
        return null;
      }
    }
